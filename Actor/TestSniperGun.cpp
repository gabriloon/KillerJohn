...
ATestSniperGun::ATestSniperGun()//사용X, 그냥 TestGun 내 변수값들 BP 마다 다르게 하는것으로 변경
{
...
void ATestSniperGun::Shoot() //ShooterCharacter의 PullTrigger함수 내에서 실행
{
	...
	AActor* HitActor = Hit.GetActor();//Hit Point 간격에 있는 Actor 
	if(HitActor!= nullptr){//Actor가 있을경우
	FPointDamageEvent DamageEvent(GunDamage,Hit,ShootRotation,nullptr);//DamageEvent 선언
...
// Called every frame
void ATestSniperGun::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);

}

bool ATestSniperGun::GunTrace(FHitResult& Hit,FVector& ShootRotation) 
...
AController* ATestSniperGun::GetThisController() const
{
